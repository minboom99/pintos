 ...
 * it was terminated by the kernel (fd.e. killed due to an
...
int process_wait(tid_t child_tid UNUSED) {
  ...
  struct thread *child = find_child_with_pid(parent, child_tid);
  int exit_status;
  ...
  if (child->is_terminated) {
    ...
    if (&child->elem != NULL && &child->elem.prev != NULL &&
        &child->elem.next != NULL) {
...
static bool setup_stack(struct intr_frame *if_) {
   ...
   * You can allocate and initialize it with the command line arguments at load time,
...
struct thread *find_child_with_pid(struct thread *parent, int child_pid) {
  ...
  if (list_empty(&parent->child_ls)) return NULL;

  e = list_front(&parent->child_ls);
  while (e != &(parent->child_ls.tail)) {
    child_thread = list_entry(e, struct thread, child_e);
    if (child_thread->tid == child_pid) return child_thread;
    e = e->next;
  }
  return NULL;
}
