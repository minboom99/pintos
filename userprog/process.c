...
#define MAX_ARG_NUM 30 // ! 나중에 문제되면 argument-passing malloc으로 하는 식으로 바꾸기
...
int process_wait(tid_t child_tid UNUSED)
{
	...
	if (child->is_terminated) {
		...
		if (&child->elem != NULL && &child->elem.prev != NULL && &child->elem.next != NULL) {
...
char *
push_argument_stack(char *parse, struct intr_frame *if_)
{
